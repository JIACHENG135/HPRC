...
%     [N,Nd] = bar_length_correction_ClassK(N,Nd,C_b,P,D,len_hat);
    ...
    % Update the stiffness of the strings
%     norm_s = zeros(size(S(1,:)));
%     for j=1:length(S(1,:))
%         norm_s(j) = norm(S(:,j));
%         forces.k(1,j) = INPUTS.E_ljc * INPUTS.Volume(j) / (norm_s(j)^2);
%     end
	...
	if ~rem(idx, 1)
		...
		logging_temp{bar_len_log}(:,idx) = diag(len_hat);
	end
	...
	if numerical_omega_solver
		cvx_begin quiet
			...
			Wsgyro_current = W + [Q*Ja_hat*gyro_omega_hat*len_hat^-2*C_bb, zeros(3,sigma)] + Omega*P';
			F = (Wsgyro_current-N*C_s'*gamma_hat*C_s);

			Jt_hat = tenseg_struct.Jt_hat;
			if ~isempty(C_b)
				lambda_hat = -Jt_hat*len_hat^-2*diag(diag(Bd'*Bd))-1/2*len_hat^-2*diag(diag(B'*F*C_b'));
			else
				lambda_hat = 0;
			end
...
%         Omega = OMEGA_LAGRANGE;
        ...
        M_bar = U2'*M*U2;
        K_bar = U2'*K*U2;
        W_bar = (W + [Q*Ja_hat*gyro_omega_hat*len_hat^-2*C_bb, zeros(3,sigma)])*U2-ETA1*U1'*K*U2;
