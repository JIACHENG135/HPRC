function dy = classkcontrol(t,x0,Inp)
...
% 
%  gamma_hat = gamma_hat_func(S,s0_motor);
...
Acon = a*eye(size(L,1));
Bcon = b*eye(size(L,1));
BTu = L*(W+Omg*P')*Minv*R + Acon*L*Nd*R + Bcon*(L*N*R-Yt);
BIGTAU=[];meu=[];
EYE = eye(size(R,2));
for i=1:size(R,2)
...
gammas = lsqlin(BIGTAU,meu,[],[],[],[],zeros(ns,1),ones(ns,1),[],options);
