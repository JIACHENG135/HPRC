function [History,info] = tenseg_sim_classkopen_old(sim_task)
...
Xhist_new = ode4(@tenseg_dyn_ckopen_fnc_old,t,X0,INPUTS);
% Extract internally logged variables and label them
%%%%%%%%%%%%%%%%%%%%%%
logs = extractSignals(@tenseg_dyn_ckopen_fnc_old,{'bar_len','gamma'});
% % 
...
ETA2dhist = reshape(Xhist2(:,1:size(Xhist2,2)/2)',3,[],numel(t));
...
%     Nddhist(:,:,i) = [ETA1 ETA2dhist(:,:,i)]*inv(U');
%     
%     Nhist(:,:,i) = [ETA1 ETA2hist(:,:,i)]*U';
%     Ndhist(:,:,i) = [zeros(size(ETA1(:,1:rank_SIGMA))) ETA2dhist(:,rank_SIGMA+1:end,i)*pinv(U2)'];
%     Nddhist(:,:,i) = [zeros(size(ETA1(:,1:rank_SIGMA))) ETA2ddhist(:,rank_SIGMA+1:end,i)*pinv(U2)'];


    Nddhist(:,:,i) = [zeros(size(ETA1)) ETA2ddhist(:,:,i)]*U';

    
%     Nhist(:,:,i) = [ETA1 ETA2hist(:,:,i)]*inv(U);
%     Ndhist(:,:,i) = [zeros(size(ETA1)) ETA2dhist(:,:,i)]*inv(U);
% %     Ndhist(:,:,i) = [ETA1 ETA2dhist(:,:,i)]*inv(U');
%     Nddhist(:,:,i) = [zeros(size(ETA1)) ETA2ddhist(:,:,i)]*inv(U);
