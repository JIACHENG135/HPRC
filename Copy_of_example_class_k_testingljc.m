...
plotStructure.N_simple=N_simple;
plotStructure.C_b=C_b;
plotStructure.C_s=C_s;
tenseg_plot(N_simple,C_b,C_s);
%%
% Convert specified class k structure into a class 1 structure with constraints
[N_new,C_b_new,C_s_new,P,node_constraints,bar_nodes_index,string_nodes_index] = tenseg_class_k_convert(N_simple,C_b,C_s,pinned_nodes);
...
s_0_b = tenseg_percent2s0(N_new,C_b_new,S_0_percent);
ms = tenseg_massive_string(s_0,N_new,C_s_new,string_nodes_index);
m = tenseg_massive_string(s_0_b,N_new,C_b_new,bar_nodes_index);
% M = 10; Vda=20;Vxiao=
% m(find(m)==min(m))=min(m)+M
...
classK_test.video = 1;
classK_test.ms = ms;
classK_test.filename = ['DC',',dt=',num2str(dt),',tf=',num2str(tf)];

% Add Velocity 
V=zeros(3,length(N_new(1,:)));
% V(3,1:length(N(1,:))/3)=-2*ones(1,length(N(1,:))/3);
V(3,1:2*pi/phi)=-Vxiao;
% V(2,1:2*pi/phi)=-1;
% *ones(1,2*pi/phi);
% classK_test.Nd0= V;
% V=zeros(3,length(N_new(1,:)));
...
%% plotA
plotA(Hist,dt,C_s_new,S_0_percent,s_0,tf,5,plotStructure)
