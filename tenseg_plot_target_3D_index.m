function [fig_out] = tenseg_plot_target_3D_index( N,C_b,C_s,target,fig_handle,highlight_nodes,view_vec )
...
if ~isempty(C_b),
	...
	for j = 1:size(B,2),
	   bar_start_nodes(:,j) = N(:,C_b(j,:)==-1);
              text(bar_start_nodes(1,j),bar_start_nodes(2,j),bar_start_nodes(3,j),int2str(j))

	end
	quiver3(bar_start_nodes(1,:),bar_start_nodes(2,:),bar_start_nodes(3,:),B(1,:),B(2,:),B(3,:),'black.','Autoscale','off','LineWidth',2)
    hold on
    for j = 1:size(B,2),
              text((bar_start_nodes(1,j)+B(1,j))/2,(bar_start_nodes(2,j)+B(2,j))/2,(bar_start_nodes(3,j)+B(3,j))/2,int2str(j)),hold on
	end
...
plot3(target_new(:,1),target_new(:,2),target_new(:,3),'ro','MarkerSize',8,'MarkerFaceColor','blue')
% Modify plot display
grid off
axis equal
if isempty(view_vec),
