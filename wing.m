...
% Specify initial node p+-ositions for class k structure
...
N=[4 0 0 0];phi=pi/18;q=4;
[full_datau,N_wingu,N_wingl,Nu,Nl,C_b_in,C_s_in]=genewing('4412',N,phi,q);
Nu(3,1) = 0;
Nu(3,end)=0;
Nl(3,1) = 0;
Nl(3,end)=0;
...
Nly=Nl(2,:);
Nlz=Nl(3,:);
N_simpleu=[Nuy;Nux;Nuz];
N_simplel=[Nly;Nlx;Nlz];
N_simple = [N_simpleu N_simplel];
...
%    C_b_in = [C_b_in;i i+1];
% e hggggggggggggggggggggggggggggggggggggggggggggggggggggggggj
...
C_s_in_l = C_b_in + length(N_simpleu(1,:));
C_b_in_l = C_s_in + length(N_simpleu(1,:));
C_b_in = [C_b_in;C_b_in_l];
C_s_in = [C_s_in;C_s_in_l];
C_s = tenseg_ind2C(C_s_in,N_simple);
C_b = tenseg_ind2C(C_b_in,N_simple);
...
% [N_simplel,C_b_l,C_s_l,parentsl] = tenseg_string_segment(N_1L,C_b_l,C_s_l,1);

% % pinned_nodes
% pinned_nodes = [1:pi/phi];
% pinned_nodes = [length(N_simpleu(1,:))];
% To illustrate the class k conversion process, print the initial number of
% nodes here
disp(['Initial # of nodes: ' num2str(size(N_simpleu,2))])
tenseg_plot(N_simple,C_b,C_s);
plotN_wing(N_wingu);
% tenseg_plot(N_simplel,C_b_l,C_s_l);
plotN_wing(N_wingl);
%%
% Convert specified class k structure into a class 1 structure with constraints
[N_new,C_b_new,C_s_new,P,node_constraints] = tenseg_class_k_convert(N_simpleu,C_b,C_s,pinned_nodes);
% [N_new,C_b_new,C_s_new,P] = tenseg_class_k_convert(N_simple,C_b,C_s);
D = zeros(3,size(P,2));
D(:,end) = N_new(:,length(N_simpleu(1,:)));
