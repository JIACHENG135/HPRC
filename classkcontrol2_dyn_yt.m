function dy = classkcontrol2_dyn_yt(t,x0,Inp)
...
Yt_temp=Inp.Yt;
round(length(time_counter)/4)
if length(time_counter)>length(Inp.Yt)
    Yt_temp = Yt_temp{1,length(Inp.Yt)};
else
    Yt_temp = Yt_temp{1,round(length(time_counter)/4)+1};
end
% length(time_counter)+1
Yt = Yt_temp';
X2 = x0(1:numel(x0)/2);
